...
class InfrastructureStatusServiceTest {
    ...
    @Test
    void reportsPreparedKafkaWithoutClaimingThatPaymentsArePublished() {
        ...
        assertThat(response.kafka().publishingEnabled()).isFalse();
        assertThat(response.webhooks().activeEndpoints()).isEqualTo(2);
        assertThat(response.pipeline().get(3).state()).isEqualTo("READY_NOT_PUBLISHING");
    }
